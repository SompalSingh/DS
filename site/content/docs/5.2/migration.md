...
- **Offcanvas component now has [responsive variations]({{< docsref "/components/side-panel#responsive" >}}).** The original `.offcanvas` class remains unchanged—it hides content across all viewports. To make it responsive, change that `.offcanvas` class to any `.offcanvas-{sm|md|lg|xl|xxl}` class.
...
- Added the new [offcanvas component]({{< docsref "/components/side-panel" >}}).
